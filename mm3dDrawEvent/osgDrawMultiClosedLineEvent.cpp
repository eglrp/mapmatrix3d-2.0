// osgDrawMultiClosedLineEvent.cpp : 定义 DLL 应用程序的导出函数。
//

#include "stdafx.h"
#include "osgDrawMultiClosedLineEvent.h"

bool COsgDrawMultiClosedLineEvent::handle(const osgGA::GUIEventAdapter &ea, osgGA::GUIActionAdapter &aa)
{
	if (isActivated)
	{
...
void COsgDrawMultiClosedLineEvent::DoubleClickDrawClosedLineOperate(osg::Vec3d worldPt, std::vector<osg::Vec3d> VecWorldCoord, osg::ref_ptr<osg::Group> editSceneGroup, osg::Matrix worldMatrix)
{
	if (VecWorldCoord.size() >= 3)								//头尾相连
	{
		osg::Vec4d color(1.0, 0.0, 1.0, 1.0);
		double size = 3.5f;
		IBaseDrawer->BaseDrawLineOperate(worldPt, vecWorldCoord[0], color, size - 2, editSceneGroup, worldMatrix);
	}

	vecWorldCoord.clear();
}

void COsgDrawMultiClosedLineEvent::UpdateActivatedStatus(std::vector<OperatingState> vecOpState)
{
	bool flag = false; 
	...
	isActivated = isActivate(vecOpState, OPS_DRAW_MULTI_CLOSED_LINE);
